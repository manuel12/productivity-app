...
test.describe("Register", () => {
  test("should allow the user to register with valid credentials", async ({
    ...
    await page.getByTestId("email").fill("testuser1@gmail.com")

    // 4. Enter password on password input
    await page.getByTestId("password").fill("Testpass1!")

    // 5. Enter passwordConfirmation on password confirmation input
    await page.getByTestId("password-confirmation").fill("Testpass1!")
...
test.describe("Login", () => {
  test.only("should allow the user to login with valid credentials", async ({
    page,
  }) => {
...
test.describe("Add todo", () => {
  ...
  test('should add a todo by writing on input and clicking on a "Add todo" button', async ({
    ...
    await page.getByTestId("todo-input").fill("Feed the cats")
...
test.describe("Edit todo", () => {
  test.beforeEach(async ({ page, request }) => {
    ...
    await loginWithAPI(request, {
      ...
      console.log(token)
      createTodoWithAPI(request, token, "Feed the cats")
    })
    await page.goto("http://localhost:3000/")
  })
  ...
  test("should edit a todo", async ({ page }) => {
    ...
    await page.getByTestId("todos-text-input").fill("Mow the lawn")

    // 4. Press ENTER key
    await page.getByTestId("todos-text-input").press("Enter")

    // 5. Check the old description text is not visible in the todo
    await expect(page.getByText("Feed the cats")).not.toBeVisible()
...
test.describe("Uncomplete todo", () => {
  test.beforeEach(async ({ page, request }) => {
    ...
    await loginWithAPI(request, {
      ...
      const responseBody = await response.json()
      const token = responseBody.token
      createTodoWithAPI(request, token, "Feed the cats", true)
    })
    await page.goto("http://localhost:3000/")
    await page.getByTestId("complete-tab").click()
  })
...
test.describe("Tabs", () => {
  test.beforeEach(async ({ page, request }) => {
    ...
    await loginWithAPI(request, {
      ...
      const token = responseBody.token
      createTodoWithAPI(request, token, "Feed the cats")
      createTodoWithAPI(request, token, "Take out trash")
      createTodoWithAPI(request, token, "Clean room")
    })
    await page.goto("http://localhost:3000/")
  })
